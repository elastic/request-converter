...
export default function Editor({ className, id, value, onChange }: EditorProps) {
  ...
  const updateCompletions = async () => {
    ...
    if (newCompletions.length) {
      ...
      completionsRef.current.style.top = `${cursorRow * 21 + 30}px`;
  ...
  const onCompletionSelected = (selection: Completion) => {
    ...
    let afterOverlap = 0;
    while (after[afterOverlap] === selection.insert[afterOverlap]) {
      afterOverlap++;
    }
    ...
    editorRef.current.value = before.slice(0, before.length - selection.replace.length) +
      selection.replace + selection.insert + after.slice(afterOverlap);
    if (onChange) {
      onChange({target: editorRef.current});
    }
    editorRef.current.selectionStart = editorRef.current.selectionEnd = cursorPos + selection.insert.length;
