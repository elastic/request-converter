...
if [[ ! -d $SCRIPT_DIR/.java ]]; then
    ...
    git clone -b "$BRANCH" --depth=1 "https://github.com/elastic/elasticsearch-java.git" .java ||
    (echo "Branch $BRANCH not found. Cloning main branch." &&
      git clone -b "main" --depth=1 "https://github.com/elastic/elasticsearch-java.git" .java)
fi
